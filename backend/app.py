...
# backend/app.py の create_customer 関数
...
def create_customer(customer: Customer):
    ...
    # 1. 挿入処理
    insert_result = crud.myinsert(mymodels_MySQL.Customers, values)
    
    # ★ ここで insert_result の中身をチェック！
    # もしエラー文字列が返ってきていたら、即座にエラーにする
    if insert_result != "inserted":
         raise HTTPException(status_code=400, detail=f"Insert Failed: {insert_result}")

    # 2. 挿入後のデータ確認
    result = crud.myselect(mymodels_MySQL.Customers, values.get("customer_id"))

    # ★ データが取れなかった場合もエラーにする（ここが重要）
    if not result:
        raise HTTPException(status_code=500, detail="Data verified failed (Save failed silently)")
